...
def terminate_all_instances(region):
    ...
    if ec2_conn:
        reservations = ec2_conn.get_all_reservations(filters={'tag:Name': NameFilter})
        #print(len(reservations.instances))
        for reservation in reservations:
            if reservation:
                for ins in reservation.instances:
                    idList.append(ins.id)
...
def launch_new_instances(region, number):
    ...
    img = ec2_conn.get_all_images(filters={'name': 'ubuntu/images/hvm-ssd/ubuntu-xenial-16.04-amd64-server-20210429'})[
        0].id
    reservation = ec2_conn.run_instances(image_id=img,  # 'ami-df6a8b9b',  # ami-9f91a5f5
                                         min_count=number,
                                         max_count=number,
                                         key_name=your_key_name,
                                         instance_type='m5.xlarge',  # 'c5.2xlarge', t2.micro（每月免费750小时） t2.medium, m5.xlarge
